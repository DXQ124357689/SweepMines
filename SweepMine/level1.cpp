    ...
    : QMainWindow{parent}
{
    ...
    setWindowTitle("Sweep Mines Level1");
    ...
    connect(Level02,&QAction::triggered,this,[=](){
        ...
        L2->setGeometry(this->geometry().x()-140,this->geometry().y()-140,780,880);
        this->hide();
        L2->show();
    });
    ...
    connect(Level03,&QAction::triggered,this,[=](){
        ...
        L3->setGeometry(this->geometry().x()-240,this->geometry().y()-200,980,1050);
        this->hide();
        L3->show();
    });
...
void Level1::paintEvent(QPaintEvent *)// 9*9 10
{
    ...
    if(currentTime == 999){ gameTime->stop(); }
    ...
    painter->drawPixmap(234,80,pix.width(),pix.height(),pix);

    if(gameStatus == 0){
        pix.load(":/res/ing.bmp");
        painter->drawPixmap(234+5,80+5,pix.width()+6,pix.height()+6,pix);
    }
...
//        qDebug()<<"("<<leftPoint.y() - 1<<" , "<<leftPoint.x() - 1<<") :"<<-(rows*rows-unPressNum);
        leftPoint = QPoint();
