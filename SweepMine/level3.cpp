    ...
    : QMainWindow{parent}
{
    ...
    setFixedSize(980,1050);
    setWindowIcon(QIcon(":/res/bomb.bmp"));
    setWindowTitle("Sweep Mines Level3");
    ...
    connect(Level01,&QAction::triggered,this,[=](){
        ...
        L1->setGeometry(this->geometry().x()+280,this->geometry().y()+200,500,600);
        this->hide();
        L1->show();
    });
    ...
    connect(Level02,&QAction::triggered,this,[=](){
        ...
        L2->setGeometry(this->geometry().x()+140,this->geometry().y()+100,780,880);
        this->hide();
        L2->show();
    });
...
bool Level3::eventFilter(QObject *watched, QEvent *event)
{
    if (watched == this) {
...
void Level3::paintEvent(QPaintEvent *)// 21*21 99
{
    ...
    painter->drawPixmap(474,80,pix.width(),pix.height(),pix);

    if(gameStatus == 0){
        pix.load(":/res/ing.bmp");
        painter->drawPixmap(474+5,80+5,pix.width()+6,pix.height()+6,pix);
    }
